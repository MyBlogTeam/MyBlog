...
public class MessageDao implements IMessageDao {
	...
	@Override
	public PageList<MessageInfo> getProcList(String showFields,
	...
	public static void main(String[] args) {
		...
		minfo.setSender(udaoDao.selectById(2));
		minfo.setReceiver(udaoDao.selectById(1));
		minfo.setMessageContent("低头思故乡");
		minfo.setMessageDate(new Timestamp(System.currentTimeMillis()));
		minfo.setMessageId(1);
		minfo.setMsgIsDel(false);
		// mdao.insert(minfo);
		...
		// 对生成对象的方法进行测试
		// MessageInfo minfo1 = mdao.selectById(1) ;
		// System.out.println("留言："+minfo.getMessageContent());
		// System.out.println(minfo1);

		// 查询所有
		// List<MessageInfo> list = mdao.selectAll();
		// for (int i = 0; i < list.size(); i++) {
		// System.out.println(list.get(i).toString());
		// }

		// 根据条件查询
		// List list = mdao.selectByWhere("msgIdDel=1");
		// for (int i = 0; i < list.size(); i++) {
		// System.out.println(list.get(i).toString());
		// }
		//
		...
		// 分页
		PageList page = mdao.getProcList("*", "messageInfo", "msgIsDel=0",
				"messageId", 3, 4);
		System.out.println("sumCount:" + page.getSumCount());
		System.out.println("pageCount:" + page.getPageCount());
		for (int i = 0; i < page.getList().size(); i++) {
			MessageInfo message = (MessageInfo) page.getList().get(i);
