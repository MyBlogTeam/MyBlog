...
public class UserDao implements IUserDao {
	...
	public boolean insert(UserInfo model) {
		...
		boolean bool = false;
		String sql = "insert into userinfo(blogId,userName,userPwd,userNickName,userSex,userBirthday," +
				"userAddress,userTel,userEmail,userPic,userRank,userState,userIsDel) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
		conn = DBUtil.getConn();
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, model.getBloginfo().getBlogId());
			ps.setString(2, model.getUserName());
			ps.setString(3, model.getUserPwd());
			ps.setString(4, model.getUserNickName());
			ps.setString(5, model.getUserSex());
			ps.setDate(6, (java.sql.Date) model.getUserBirthday());
			ps.setString(7, model.getUserAddress());
			ps.setString(8, model.getUserTel());
			ps.setString(9, model.getUserEmail());
			ps.setString(10, model.getUserPic());
			ps.setInt(11, model.getUserRank());
			ps.setInt(12, model.getUserState());
			ps.setBoolean(13, model.getUserIsDel());
	...
	public boolean update(UserInfo model) {
		...
		PreparedStatement ps = null;
		boolean bool = false;
		String sql = "update userinfo set blogId=?,userName=?,userPwd=?,userNickName=?,userSex=?,userBirthday=?,"
		...
		try {
			...
			ps.setString(4, model.getUserNickName());
			ps.setString(5, model.getUserSex());
			ps.setDate(6, (Date) model.getUserBirthday());
			ps.setString(7, model.getUserAddress());
			ps.setString(8, model.getUserTel());
			ps.setString(9, model.getUserEmail());
			ps.setString(10, model.getUserPic());
		...
		try {
			if (rs != null) {
				while (rs.next()) {
					...
					user.setBloginfo(new BlogDao().selectById(rs
							.getInt("blogId")));
...
//		BlogInfo bi = new BlogInfo(1001, ui, "hello", "hello", "2000-1-6", 1,
//				"生如夏花", "", false);
//		ui.setBloginfo(bi);
